...
export default function StudentProfile() {
  ...
  // Detectar se é primeiro acesso (cadastro incompleto)
    ...
    const handleCpfChange = (e) => {
        ...
        try {
            setAvatarFile(file);
            // Try immediate upload to persist and preview
            try {
                const { localApi } = await import('@/api/localApi');
                const up = await localApi.uploadFile(file);
                if (up?.url) {
                    setValue('avatar_url', up.url, { shouldDirty: true });
                    setAvatarPreview(up.url);
                } else {
            ...
            console.error('Erro no upload da foto:', error); // Corrigido
                            ...
                            <div className="md:col-span-1 flex flex-col items-center">
                                <div className="relative">
                                    <Avatar className="h-24 w-24 md:h-32 md:w-32">
                                        {formData.avatar_url ? (
                                            <AvatarImage src={avatarPreview || formData.avatar_url} alt="Foto de perfil" className="object-cover" />
                                        ) : (
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="space-y-2">
                                    ...
                                    {isEditing ? (
                                        ...
                                        <p className="p-2 bg-gray-50 rounded border">{formData.cep || "Não informado"}</p>
                                    )}
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="space-y-2">
                                    ...
                                    {isEditing ? (
                                        <Input id="address_number" name="address_number" {...register("address_number", { required: "Número é obrigatório" })} className="border-orange-200" />
                                    ) : (
                                        <p className="p-2 bg-gray-50 rounded border">{formData.address_number || "Não informado"}</p>
                                    )}
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="neighborhood">Bairro</Label>
                                    {isEditing ? (
                                        <Input id="neighborhood" name="neighborhood" {...register("neighborhood", { required: "Bairro é obrigatório" })} className="border-orange-200" />
                                    ) : (
                                        <p className="p-2 bg-gray-50 rounded border">{formData.neighborhood || "Não informado"}</p>
                                    )}
                                </div>
                                <div className="space-y-2">
